...
Public Class ConsultaAccidenteLugar
    ...
    Private Sub ConsultaAccidenteLugar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Label3.Visible = False
        fecha.Visible = False
        Label3.Enabled = False
        fecha.Enabled = False
    ...
    Private Sub busc_Click(sender As Object, e As EventArgs) Handles busc.Click
        If filtro.SelectedItem = "fecha" Then
            ABRIRCONEXION()
            Dim cmd As New SqlCommand("Select * from Accidente where fechaccidente = '" + fecha.Value.Date + "'", CN)
        ...
        If filtro.SelectedItem = "hora" Then
            ABRIRCONEXION()
            Dim cmd As New SqlCommand("Select * from Accidente where horaccidente = '" + hora.Text + "'", CN)
            Dim adp As New SqlDataAdapter(cmd)
            'Dim cmd2 = New SqlCommandBuilder(adp)
            Dim tabla As New DataTable
            adp.Fill(tabla)
